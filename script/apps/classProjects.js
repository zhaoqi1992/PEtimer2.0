$(function() {
	...
	var projects = [];
	if (localStorage.getItem(className + '-project')) {
		projects = localStorage.getItem(className + '-project').split(',');
	}
	...
	var count = {
		init: function(projects) {
			...
			for (var i = 0; i < projects.length; i++) {
				...
				str += "<td>" + className + projects[i] + "</td>";
		...
		bind: function() {
			...
			//点击添加班级按钮，跳转到添加班级页面
			$('.classes-control-functions-histoyr').on('click', function() {
				window.location = '#';
			});
